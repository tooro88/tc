...
(defun tctest-is-non-im ()
  ...
  (memq tcode-use-isearch '(t overwrite advice)))

(defun tctest-is-overwrite ()
  "`overwrite' の isearch 実装を使っている。"
  (memq tcode-use-isearch '(t overwrite)))

(defun tctest-has-wrapped-search ()
  "wrapped search が実装されている。"
  (or (tctest-is-overwrite)
      (and (memq tcode-use-isearch '(advice im))
...
(ert-deftest tctest-wrapped-search ()
  ...
  :expected-result (if (tctest-has-wrapped-search) :passed :failed)
  (skip-unless tcode-use-isearch)
...
(ert-deftest tctest-lax-whitespace ()
  ...
  :expected-result (if (tctest-is-overwrite) :failed :passed)
...
(ert-deftest tctest-isearch-backward-regexp-with-arg ()
  ...
  :expected-result (if (and (tctest-has-wrapped-search)
			    (not (tctest-is-overwrite)))
		       :passed :failed)
  (skip-unless tcode-use-isearch)
